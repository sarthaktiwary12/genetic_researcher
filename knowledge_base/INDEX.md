# ExRNA Biology Research Engine - Master Index
> TL;DR: Knowledge base for investigating how bacterial extracellular small RNAs improve spinach germination. 109 predicted gene targets organized by pathway and priority.
> Last Updated: 2026-02-18

## Research Question
Which spinach transcripts, if downregulated by bacterial exRNAs (antisense), explain improved germination and seedling vigor?

## Knowledge Base Structure
| Directory | Contents | Status |
|-----------|----------|--------|
| [targets/](targets/INDEX.md) | Individual gene target analyses | 109 targets analyzed |
| [pathways/](pathways/INDEX.md) | Pathway-level groupings | 14 pathways |
| [themes/](themes/INDEX.md) | Cross-cutting biological themes | 6 themes |
| [synthesis/](synthesis/) | Final ranked targets, causal models, validation plan | Pending |
| [research/](research/INDEX.md) | Raw Gemini research outputs | See log |

## Pipeline Status
- [x] Stage 1: Batch Gene Analysis
- [x] Stage 2: Pathway Mapping
- [x] Stage 3: Literature Deep-Dive
- [x] Stage 4: Theme Extraction
- [ ] Stage 5: Claude Synthesis


## Quick Stats
- Total targets: 109
- High priority: 21
- Medium priority: 49
- Low priority: 39
- Pathways covered: 14
- Organism: Spinacia oleracea (spinach)
- Treatment: M-9 bacterial EPS solution
