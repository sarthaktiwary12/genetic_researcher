...
def load_all_summaries() -> str:
    ...
    for priority_dir in ["high_priority", "medium_priority", "low_priority"]:
        dir_path = KB_TARGETS / priority_dir
...
def load_pathway_summaries() -> str:
    """Load summaries from pathway analysis files."""
    summaries = []
    if not KB_PATHWAYS.exists():
        return "(No pathway analyses available)"

    for f in sorted(KB_PATHWAYS.glob("*.md")):
        if f.name == "INDEX.md":
            continue
        try:
            content = f.read_text()
...
async def extract_themes(client: GeminiClient, gene_summaries: str) -> dict:
...
"""
        ...
        KB_SYNTHESIS.mkdir(parents=True, exist_ok=True)
        full_themes_path = KB_SYNTHESIS / "theme_analysis_full.md"
...
async def build_causal_models(
    client: GeminiClient, gene_summaries: str, pathway_summaries: str
) -> dict:
    ...
    try:
        ...
        KB_SYNTHESIS.mkdir(parents=True, exist_ok=True)
        filepath = KB_SYNTHESIS / "causal_models.md"
...
async def analyze_confounders(client: GeminiClient) -> dict:
    """Analyze confounding variables."""
    context = """Experiment: Spinach (Spinacia oleracea) seeds soaked in bacterial EPS solution "M-9" (4-8h until imbibition).
Control: Water-soaked seeds.
Phenotype: Improved germination rate, vigor, early seedling growth.
ExRNAs extracted from M-9 EPS, sequenced, antisense-aligned to spinach transcriptome.
...
Key concern: Distinguishing exRNA effects from EPS/polysaccharide/microbiome effects."""
    ...
    try:
        ...
        KB_SYNTHESIS.mkdir(parents=True, exist_ok=True)
        filepath = KB_SYNTHESIS / "confounders.md"
...
async def design_validation_plan(
    client: GeminiClient, themes: str, causal_models: str
) -> dict:
    ...
    try:
        ...
        KB_SYNTHESIS.mkdir(parents=True, exist_ok=True)
        filepath = KB_SYNTHESIS / "validation_plan.md"
...
async def run(client: GeminiClient) -> dict:
    ...
    gene_summaries = load_all_summaries()
    pathway_summaries = load_pathway_summaries()
    ...
    theme_result, causal_result, confounder_result = await asyncio.gather(
        extract_themes(client, gene_summaries),
        build_causal_models(client, gene_summaries, pathway_summaries),
        analyze_confounders(client),
    )
    ...
    validation_result = await design_validation_plan(client, themes_text, causal_text)

    duration = (datetime.now() - start_time).total_seconds()

    # Update theme index
    if theme_result["status"] == "success":
        update_themes_index(theme_result["themes"])
